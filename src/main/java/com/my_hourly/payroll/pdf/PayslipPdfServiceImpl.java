...
public class PayslipPdfServiceImpl implements PayslipPdfService {
 ...
 // Section 9: Generate payslip for logged-in employee (if employee role)
    @Override
    public byte[] generatePayslip(int month, int year) {

        Employee employee = employeeService.getCurrentEmployee();

        // Section 10: Validate inputs
        if (month < 1 || month > 12) {
            throw new BadRequestException(
        ...
        if (year < 2000) {
            throw new BadRequestException(
                    "A valid year is required to generate payslip.",
                    ErrorCode.BAD_REQUEST);
        }

        // Section 11: Fetch the active payroll for this employee/month/year
        LocalDate startDate = LocalDate.of(year, month, 1);
        LocalDate endDate = startDate.withDayOfMonth(startDate.lengthOfMonth());

        Payroll payroll = payrollRepository
                .findFirstByEmployeeIdAndPayrollMonthBetweenAndActiveTrue(
                        employee.getId(),
                        startDate,
                        endDate)
                .orElseThrow(() ->
                        new ResourceNotFoundException(
                                "Payroll not found for employee id: " + employee.getId()
                                        + " for " + month + "/" + year,
                                ErrorCode.RESOURCE_NOT_FOUND));

        // Section 12: Payslip PDF only available for APPROVED or PAID payrolls
        if (payroll.getStatus() != PayrollStatus.APPROVED
                && payroll.getStatus() != PayrollStatus.PAID) {
            throw new BadRequestException(
                    "Payslip can only be generated for APPROVED or PAID payrolls. "
