...
All six Settings endpoints share the same role set:
...
Default values expected on a fresh environment:
...
The following fields have no default value: `website`, `addressLine1`, `addressLine2`, `city`, `state`, `country`, `postalCode`, `logoUrl` (all stay `null`). There is **no "reset to defaults" API** — the table is for reference/placeholders only.
