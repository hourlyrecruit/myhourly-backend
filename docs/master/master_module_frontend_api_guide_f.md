...
- [4. Payload reference](#4-payload-reference)
- [5. Endpoint-by-endpoint details](#5-endpoint-by-endpoint-details)
- [6. Typical UI flows](#6-typical-ui-flows)
- [7. Error handling table](#7-error-handling-table)
- [8. TypeScript contracts](#8-typescript-contracts)
- [9. API client examples](#9-api-client-examples)
- [10. Client-side validation helper](#10-client-side-validation-helper)
- [11. Backend quirks, defects and gotchas](#11-backend-quirks-defects-and-gotchas)
- [12. Integration checklist](#12-integration-checklist)
- [13. Postman / curl smoke sequence](#13-postman--curl-smoke-sequence)
- [14. Files this guide was derived from](#14-files-this-guide-was-derived-from)

### At a Glance

| Area | Details |
|---|---|
| **Resources** | Department → Designation → Job Title |
| **Base paths** | `/api/v1/departments` · `/api/v1/designations` · `/api/v1/job-titles` · `/api/v1/lookups` |
| **Management roles** | `HR_ADMIN`, `MANAGER` |
| **Lookup access** | Any authenticated user |
| **Pagination** | 0-based, default `page=0`, `size=10` |
| **Status change** | `PATCH .../status?active=<boolean>` |
| **Primary recommendation** | Prefer **Deactivate** over **Delete** for referenced records |
...
### 11.6 ⚠️ **`PUT` silently deactivates when `active` is omitted**
...
> **1. Always send `active` on `PUT`.** Omitting it can silently deactivate the record.
>
> **2. Always send lookup IDs twice.** For dependent lookups, the ID is required both in the path and as a query parameter.
>
> **3. Prefer Deactivate over Delete.** Deleting referenced master data can surface as a `500`.
>
> **4. Keep dropdowns cascading.** Clear and reload child values whenever the parent changes.
>
> **5. Validate before calling the API.** Several avoidable input problems currently surface as `500` responses.
